...
from app.matching_ground.schema.interest import (
    BaseResponse,
    InterestReponse,
    UserInterestCreate,
    UserInterestReponse,
...
@router.get(
    "/available", status_code=status.HTTP_200_OK, response_model=InterestReponse
)
async def get_available_interests(db: dbSessionDep, _: CurrentUser):
...
async def get_user_interests(db: dbSessionDep, current_user: CurrentUser):
    return await interest_service.get_user_interests(db, str(current_user.id))


@router.get(
    "/user/{user_id}",
    response_model=UserInterestReponse,
    status_code=status.HTTP_200_OK,
)
async def get_user_interest_by_id(user_id: UUID, db: dbSessionDep, _: CurrentUser):
...
async def remove_user_interests(
    ...
    return await interest_service.remove_user_interest(
        db, str(current_user.id), interest_lists.interests
    )
