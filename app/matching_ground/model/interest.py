...
class Interest(BaseModel):
    ...
    users: Mapped[User] = relationship(
        back_populates="interests", secondary="user_interests", viewonly=True
    )

    @classmethod
    async def create_interests(
        cls, db: AsyncSession, interest_list: dict[str, list[str]]
    ) -> bool:
        data = []
        ...
        query = insert(cls).values(data).on_conflict_do_nothing(index_elements=["name"])
