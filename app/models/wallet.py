...
from sqlalchemy import DateTime, Enum, Float, ForeignKey, func, select
...
if TYPE_CHECKING:
    from app.models import OrderItem, Store, User, WalletTransaction
...
class Refund(BaseModel):
    ...
    status: Mapped[str] = mapped_column(
        Enum("pending", "released", "failed", name="refund_status"),
        default="pending",
        nullable=False,
    )
    ...
    order_item: Mapped[OrderItem] = relationship(
        back_populates="refunds", uselist=False
    )
    ...
    async def release_refund(db: AsyncSession, wallet_id: str):
        ...
        stmt = (
            select(
                func.coalesce(
                    func.sum(
                        func.case((Refund.release_at <= now, Refund.amount), else_=0.0)
                    ),
                    0.0,
                ).label("release_amount"),
                func.coalesce(
                    func.sum(
                        func.case((Refund.release_at > now, Refund.amount), else_=0.0)
                    ),
            ...
            ).where(Refund.wallet_id == wallet_id, Refund.status == "pending"),
        )
