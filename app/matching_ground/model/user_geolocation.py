...
class UserGeolocation(BaseModel):
    ...
    @classmethod
    async def get_by_user_id(cls, session: AsyncSession, user_id: uuid.UUID) -> Self | None:
        result = await session.execute(select(cls).where(cls.user_id == user_id))
        return result.scalar_one_or_none()

    @classmethod
    async def upsert(cls, session: AsyncSession, user_id: uuid.UUID, lat: float, lon: float) -> Self:
