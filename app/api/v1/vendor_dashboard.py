...
async def fetch_vendor_orders(
    ...
    order_by: str = Query("date", regex="^(name|date|price)$", description="Sort field"),
    order_dir: str = Query("desc", regex="^(asc|desc)$", description="Sort direction (asc/desc)")
) -> JSONResponse:
...
async def vendor_customers_dashboard(
    ...
    current_user: CurrentStore,
    page: int = Query(1, ge=1, description="Page number for pagination"),
    page_size: int = Query(10, ge=1, le=100, description="Number of customers per page"),
):
    return await VendorDashBoardService.get_vendor_customers(session, current_user, page, page_size)