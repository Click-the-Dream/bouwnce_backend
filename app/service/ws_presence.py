...
class PubSubDispatcher:
    ...
    async def start(self, redis) -> None:
        ...
        if inspect.isawaitable(self._pubsub):
            self._pubsub = await self._pubsub
    ...
    async def _dispatch_loop(self) -> None:
        ...
        try:
            ...
            while inspect.isawaitable(messages):
...
class PresenceManager:
    ...
    async def _catchup_chat_stream(
        ...
        *,
        ...
        async def default_should_deliver(payload: dict) -> bool:
            handler = getattr(self, "_should_deliver_chat_message", None)
            if handler is None:
                return True
            return await handler(redis=redis, user_id=user_id, payload=payload)

        async def default_send_json(ws, payload: dict, lock) -> bool:
            handler = getattr(self, "_send_json_safe", None)
            if handler is not None:
                return await handler(ws, payload, send_lock=lock)
            try:
                if lock is None:
        ...
        _should_deliver = should_deliver or default_should_deliver
        _send_json = send_json or default_send_json
