...
def within_radius(
    center: Coordinates, candidate: Coordinates, radius_km: float
) -> bool:
    return haversine_km(center, candidate) <= radius_km


def fuzz_location(
    coord: Coordinates, max_meters: int, seed: int | None = None
) -> Coordinates:
    if max_meters <= 0:
        return coord
    ...
    delta_lon = (rng.random() * 2 - 1) * (
        max_km / (111.320 * math.cos(math.radians(coord.lat)) or 1)
    )
